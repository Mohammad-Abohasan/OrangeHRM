...
describe('Login Page', () => {
    ...
    it('Check empty userName and valid password', () => {
        cy.get('@logInfo').then((infoData: any) => {
            ...
            loginPage.checkUserNameInputErrorMessageContainsValue(infoData.errorMsg.fieldRequired);
        });
    });
    ...
    it('Check valid userName and empty password', () => {
        cy.get('@logInfo').then((infoData: any) => {
            ...
            loginPage.checkPasswordInputErrorMessageContainsValue(infoData.errorMsg.fieldRequired);
        });
    });
    ...
    it('Check userName sensitivity', () => {
        cy.get('@logInfo').then((infoData: any) => {
            if (infoData.userName.valid == infoData.userName.valid.toUpperCase())
                loginPage.login(infoData.userName.valid.toLowerCase(), infoData.password.valid);
            else
                loginPage.login(infoData.userName.valid.toUpperCase(), infoData.password.valid);
            loginPage.checkValidLogin(infoData.validLogin);
            loginPage.logout();
        });
    });
    ...
    it('Check password sensitivity', () => {
        cy.get('@logInfo').then((infoData: any) => {
            if (infoData.password.valid == infoData.password.valid.toUpperCase())
                loginPage.login(infoData.password.valid.toLowerCase(), infoData.password.valid);
            else
                loginPage.login(infoData.password.valid.toUpperCase(), infoData.password.valid);
            loginPage.checkInvalidLogin(infoData.errorMsg.invalidLogin);
        });
    });
});