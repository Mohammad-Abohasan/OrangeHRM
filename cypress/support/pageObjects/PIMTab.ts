...
class PIMTab {
  ...
  searchEmployee([id, firstName, middleName, lastName, jobTitle, employmentStatus, subUnit, supervisor]: any) {
    this.openPIMTab();
    this.elements.employeeSearchId().type(id);
    this.elements.searchBtn().click({ force: true });
    this.elements.resultData().eq(1).should('contain', id);
    this.elements.resultData().eq(2).should('contain', firstName + ' ' + middleName);
